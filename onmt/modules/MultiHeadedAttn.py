...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, key, value, query, mask=None):
        ...
        aeq(batch, batch_)
        aeq(d, d_)
        aeq(self.model_dim % self.head_count, 0)
        if mask is not None:
        ...
        value_up = shape_projection(self.linear_values(value)) # (batch*head, srclen+49, dim/head)
        query_up = shape_projection(self.linear_query(query)) # (batch*head, tgtlen, dim/head)

        scaled = torch.bmm(query_up, key_up.transpose(1, 2))
        scaled = scaled / math.sqrt(self.dim_per_head)
        bh, l, dim_per_head = scaled.size() # here dim_per_head is mismeans , should be srclen
        ...
        attn = self.sm(scaled) # (batch*head, tgtlen, srclen)
        # Return one attn
        top_attn = attn \
            .view(b, self.head_count, l, dim_per_head)[:, 0, :, :] \
            .contiguous()

        # for specific case visulization
        # top_attn = attn.view(b, self.head_count, 1, dim_per_head).contiguous()
