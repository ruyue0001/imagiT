...
import onmt
from onmt.GLAttention import GLAttentionGeneral as ATT_NET
...
class CNN_ENCODER(nn.Module):
    def __init__(self, nef):
        ...
        print('Load pretrained CNN model from ', url)
        # print(model)
...
class NEXT_STAGE_G(nn.Module):
    ...
    def _make_layer(self, block, channel_num):
        layers = []
        for i in range(self.num_residual): # 2
            layers.append(block(channel_num))
        return nn.Sequential(*layers)
    ...
    def forward(self, h_code, c_code, word_embs, mask):
        ...
        #print(h_code.shape, c_code.shape, word_embs.shape)
        #h_code[batch, 32, 64, 64], c_code[batch, 100]
        c_code, weightedSentence, att, sent_att = self.att(h_code, c_code, word_embs)
        #c_code [batch, 32, 64, 64], weightedSentence [batch, 32, 64, 64], att [batch, length, 64, 64], sent_att[batch, 32, 64, 64]
        #print (c_code.shape, weightedSentence.shape, att.shape, sent_att.shape)
...
class G_NET(nn.Module):
    ...
    def forward(self, z_code, sent_emb, word_embs, mask):
        ...
        if self.BRANCH_NUM > 0:
            ...
            #print (z_code.shape, sent_emb.shape, word_embs.shape, c_code.shape, h_code1.shape, fake_img1.shape)
            fake_imgs.append(fake_img1)
            # h_code1 [batch, 32, 64, 64]
            # fake_img1 [batch, 3, 64, 64]
        if self.BRANCH_NUM > 1:
            h_code2, att1 = \
                self.h_net2(h_code1, c_code, word_embs, mask)
            fake_img2 = self.img_net2(h_code2)
            #print (h_code2.shape, fake_img2.shape)
            # h_code2 [batch, 32, 128, 128]
            # fake_img2 [batch, 3, 128, 128]
